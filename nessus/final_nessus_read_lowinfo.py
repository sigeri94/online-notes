...
def parse_nessus_file(nessus_file):
    ...
    for report_host in tree.xpath("//ReportHost"):
        ...
        for report_item in report_host.xpath(".//ReportItem"):
            ...
            if any(keyword in vuln_name.lower() for keyword in ["basic","cleartext","completion","redis","oracle","jboss","java","version", "cgi", "xss", "html injection", "apache", "php","hsts","ssl","tls","openssl","browsable","trace","clickjacking","disclosure","telnet","ssh","openssh","mDNS","identification","xxe","jenkins"]):
                plugin_output = "skipped"

            # Only include Low and Info severities
