...
def save_to_docx(vulnerabilities, output_filename):
    ...
    for ip_address, vuln_name, severity, plugin_output, solution in vulnerabilities:
        findings_by_ip[ip_address].append((vuln_name, severity, plugin_output, solution))

    # Add findings grouped by IP Address
    for ip_address, findings in sorted(findings_by_ip.items()):
        # Add IP Address Header
        doc.add_heading(f"IP Address: {ip_address}", level=3)

        # Add a table for findings under this IP Address
        ...
        hdr_cells[2].text = "Plugin Output"
        hdr_cells[3].text = "Solution"

        for vuln_name, severity, plugin_output, solution in findings:
            row_cells = table.add_row().cells
            row_cells[0].text = vuln_name
            row_cells[1].text = severity
            row_cells[2].text = plugin_output
            row_cells[3].text = solution

        # Add spacing after each IP Address section
        doc.add_paragraph("\n")
